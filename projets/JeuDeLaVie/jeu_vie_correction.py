...
def compte_voisins_vivants(grille,coord):
    ...
    for monvoisin in les_voisins:
        levoisin = grille[monvoisin[0]][monvoisin[1]]
        if levoisin == 1:
            nb_voisins_vivants +=1
    return nb_voisins_vivants
...
def grille_voisins(grille):
    ...
    for lig in range(len(grille)):
        ...
        for col in range (len(grille)):
            nb_v = compte_voisins_vivants(grille,(lig,col))
            ligne.append(nb_v)
...
def update_grille(grille):
    ...
    renvoie la grille modifiée'''
    les_voisins = grille_voisins(grille)
    for lig in range(len(grille)):
        for col in range(len(grille)):
            if grille[lig][col] == 1:
                if les_voisins[lig][col] ==2 or les_voisins[lig][col] ==3:
                    grille[lig][col] = 1
                else:
                    grille[lig][col] = 0
            else:
                if les_voisins[lig][col] ==3:
                    grille[lig][col]=1
    return grille
